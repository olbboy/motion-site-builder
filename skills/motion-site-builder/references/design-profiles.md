...
- The **prompt library** (`motion_find_reference`, 14 original prompts) is cinematic-led (6 cinematic prompts). Each non-cinematic profile ships **two curated exemplar prompts** (see `prompts/README.md` → "Profile exemplars"), retrievable by including the profile name in the query (e.g. `motion_find_reference("product-ui dashboard")`) — their catalog category carries the profile name. Use the exemplar as the structural reference, then build values from tokens + patterns + the profile primitives above; don't adapt a cinematic prompt into another profile.
